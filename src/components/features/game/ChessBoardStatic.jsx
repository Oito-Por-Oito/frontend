...
/**
 * ChessBoardStatic - Tabuleiro estático para exibição
 * Usado em páginas de seleção de modo de jogo
 */
export default function ChessBoardStatic({ size = "default" }) {
  ...
  return (
    <div className="relative my-2">
      <div className={`grid grid-cols-8 ${sizeClasses[size]} border-4 border-gold rounded-2xl shadow-2xl overflow-hidden`}>
        {[...Array(64)].map((_, i) => {
          const x = i % 8;
      ...
      <div className="absolute bottom-[-24px] left-0 w-full grid grid-cols-8 text-center text-sm text-foreground/70 font-medium">
      ...
      <div className="absolute top-0 left-[-20px] h-full grid grid-rows-8 text-sm text-foreground/70 font-medium">
