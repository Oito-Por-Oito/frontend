...
/**
 * RankingList - Lista de rankings com filtros
 */
export default function RankingList({ maxPlayers = 10 }) {
  ...
  return (
    <Card variant="elevated">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {players.length > 0 ? (
            players.slice(0, maxPlayers).map((player, idx) => (
              <tr key={player.name} className="hover:bg-surface-secondary/50">
                <td className="py-2 px-2 font-bold text-gold">{idx + 1}</td>
                ...
                <td className="py-2 px-2 text-right font-bold text-gold">
                  {player.rating}
                </td>
              </tr>
            ))
